...
class Query:
    ...
    def events_by_user(self, user_id: str, event_date: Optional[date] = None, limit: int = 100) -> list[Event]:
        ...
        rows = get_db().execute(
            "SELECT * FROM events_by_user_day WHERE user_id=%s AND event_date=%s LIMIT %s",
            (user_id, d, min(limit, 500)),
        )
    ...
    def events_by_ip(self, ip: str, event_date: Optional[date] = None, limit: int = 100) -> list[Event]:
        d = event_date or datetime.now(timezone.utc).date()
        rows = get_db().execute(
            "SELECT * FROM events_by_ip_day WHERE ip=%s AND event_date=%s LIMIT %s",
            (ip, d, min(limit, 500)),
        )
        return rows_to_events(rows)
    ...
    def events_by_device(self, device_id: str, event_date: Optional[date] = None, limit: int = 100) -> list[Event]:
        d = event_date or datetime.now(timezone.utc).date()
        rows = get_db().execute(
            "SELECT * FROM events_by_device_day WHERE device_id=%s AND event_date=%s LIMIT %s",
            (device_id, d, min(limit, 500)),
        )
        return rows_to_events(rows)
    ...
    def investigations_by_user(self, user_id: str, limit: int = 20) -> list[Investigation]:
        rows = get_db().execute(
            "SELECT * FROM investigations_by_user WHERE user_id=%s LIMIT %s",
...
@app.get("/")
def root():
    return {"name": "SentinelFlow", "graphql": "/graphql"}


@app.get("/health")
def health():
    return {"status": "ok", "cassandra_hosts": HOSTS}
